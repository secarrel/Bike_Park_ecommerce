...
def add_to_basket(request, item_id):
    """ Add a timeslot and quantity of the specified activity to the shopping bag """
        ...
        if timeslot.available_capacity < quantity :
            ...
            if timeslot_id in basket:
                ...
                if sumQuantity > timeslot.available_capacity:
                    messages.error(request, f'There are not enough spaces available for the number you are adding and those already in your basket')
                else:
        ...
        if quantity != currentQuantity:
            ...
            if quantity > 0:
                if quantity <= timeslot.available_capacity:
                    ...
                    messages.success(request, f'{timeslot.activity} quantity has been updated to {quantity}')
                else: 
                    messages.error(request, f'Can not update quantity. There are only {timeslot.available_capacity} spaces available to book.')
            else:
                basket.pop(item_id, None)
                messages.info(request, f'Quantity is "0" so {timeslot.activity} has been removed from your basket')
            request.session['basket'] = basket
...
def remove_from_basket(request, item_id):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            request.session['basket'] = basket
            timeslot = get_object_or_404(Timeslot, pk=item_id)
            messages.success(request, f'{timeslot.activity} has been removed from your basket')
            return HttpResponse(status=200)
        except Exception as e:
            messages.error(request, f'Error removing {timeslot.activity} from basket')
